...
export default {
  ...
  computed: {
    rootLink() {
      return this.$route.name === 'blog' ? '/blog' : '/blog_new/';
    },

    /* blogImage() {
      // /upload/articles/439672c840930dd9.jpg
      let url = this.data.img || this.data.image;
      let arr = url.split('/');
      [,, ...arr] = arr;
      url = arr.join('_');
    ...
    link() {
      return `${this.rootLink}/${this.data.url}`;
    },
  },
  beforeUpdate() {
    this.getImgSize();
  ...
  methods: {
    getImgSize() {
      const gridsize = this.$refs.gridItem.offsetWidth;
      const query = `/image/resize_${gridsize}x${gridsize}/`;
      let url = this.data.img || this.data.image;
      let arr = url.split('/');
      [,, ...arr] = arr;
      url = arr.join('_');
      this.blogImage = query + url;
    },
  },
