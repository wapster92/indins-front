...
export default {
  ...
  methods: {
    reportLink() {
      return `/api/calltracking/report/type?from=${this.filterModel[0]}&to=${this.filterModel[1]}`;
    },
    download() {
    ...
    async getData() {
      ...
      const request = new Simple(
        ...
        $nuxt.context.isDev
          ? `http://localhost:8000/api/calltracking/chart/type?from=${this.filterModel[0]}&to=${this.filterModel[1]}${this.uniqid ? `&uniqid=${this.uniqid}` : ''}`
          : `/api/calltracking/chart/type?from=${this.filterModel[0]}&to=${this.filterModel[1]}${this.uniqid ? `&uniqid=${this.uniqid}` : ''}`,
        {},
        'GET',
        false,
